...
mock_script_ctx = MagicMock()
mock_script_ctx.session_id = "test_session_123"
mock_get_ctx = mock_st.runtime.scriptrunner_utils.script_run_context.get_script_run_ctx
mock_get_ctx.return_value = mock_script_ctx
...
class TestFullSessionPersistence:
    ...
    def setup_method(self):
        ...
        # Patch configuration
        persistence_path = 'webapp.utilities.session.session_persistence.config_manager'
        backend_path = 'webapp.utilities.storage.sqlite_session_backend.config_manager'
        corpus_path = 'webapp.utilities.corpus.data_manager.config_manager'
        
        self.config_patcher = patch(persistence_path, self.mock_config)
        self.config_patcher2 = patch(backend_path, self.mock_config)
        self.config_patcher3 = patch(corpus_path, self.mock_config)
        
        self.config_patcher.start()
        self.config_patcher2.start()
        self.config_patcher3.start()
    ...
    def test_basic_session_initialization_and_persistence(self):
        ...
        assert self.session_id in mock_st.session_state
        assert "session" in mock_st.session_state[self.session_id]
        
        # Verify session was persisted to database
        session_data = self.backend.get_session(self.session_id)
    ...
    def test_session_updates_with_persistence(self):
        ...
        session_df = mock_st.session_state[self.session_id]["session"]
        session_dict = session_df.to_dict(as_series=False) if hasattr(session_df, 'to_dict') else session_df
        
        assert session_dict.get("HAS_TARGET") == [True] or session_dict.get("HAS_TARGET") == True
        assert session_dict.get("TARGET_DB") == ["test_corpus"] or session_dict.get("TARGET_DB") == "test_corpus"
    ...
    def test_corpus_data_persistence(self):
        ...
        data_manager = CorpusDataManager("target", self.session_id)
        data_manager.set_core_data(test_data)
        
        # Verify data in session state
        assert "target" in mock_st.session_state[self.session_id]
        assert "ds_tokens" in mock_st.session_state[self.session_id]["target"]
    ...
    def test_ai_assistant_persistence(self):
        ...
        assert self.session_id in mock_st.session_state
        assert "messages" in mock_st.session_state[self.session_id]
        assert "plot_intent" in mock_st.session_state[self.session_id]
        
        # Add a test message
        mock_st.session_state[self.session_id]["messages"].append({
            "role": "user",
    ...
    def test_cross_session_continuity(self):
        ...
        session_df = mock_st.session_state[self.session_id]["session"]
        session_dict = session_df.to_dict(as_series=False) if hasattr(session_df, 'to_dict') else session_dict
        ...
        # Check AI messages
        messages = mock_st.session_state[self.session_id]["messages"]
    ...
    def test_generate_temp_with_persistence(self):
        ...
        # Verify in memory
        assert "test_key1" in mock_st.session_state[self.session_id]
        assert "test_key2" in mock_st.session_state[self.session_id]
        assert "test_key3" in mock_st.session_state[self.session_id]
    ...
    def test_database_structure_and_integrity(self):
        ...
        cursor.execute("SELECT session_id, data FROM sessions WHERE session_id = ?", (self.session_id,))
        row = cursor.fetchone()
